...
%         u0 = f2(l_now);
    ...
    elseif (i>mm_real(11) && i<=mm_real(12))
        ...
        u0 = 195;
        A(1,1:2) = [h*ke/lemda+1 -1];
        A(ms+1,ms:ms+1) = [ -1 h*ke3/lemda+1];
        f(1) = h*ke*u0/lemda;
        f(ms+1) = h*ke3*u0/lemda;
    elseif (i>mm_real(12) && i<=mm_real(13))
        %空隙6
        l_now = (i-mm_real(12))/100*70/60;
        u0 = f3(l_now);
        A(1,1:2) = [h*ke3/lemda+1 -1];
...
% plot(x(idx),q(idx),'*')
% plot(x,q)
plot(x,q,'b','linewidth',1)
hold on
plot(test_x,test_y,'r','linewidth',1)
title('实验数据与附件数据对比')
legend('实验数据','附件数据')
xlabel('时间/s')
ylabel('温度/摄氏度')
